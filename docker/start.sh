...
echo "starting event api on pm2"
pm2 start /app/api/event.js

echo "starting event-handler on pm2"
pm2 start /app/bin/handle.js

echo "starting http-server for ui on pm2 (for apidoc)"
